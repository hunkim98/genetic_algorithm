...
// Reference: https://github.com/ResoluteError/ts-neural-network
...
export class NNNode implements NNNodeObj {
  ...
  calculateValue(inputActivations: number[]): number {
    if (inputActivations.length !== this.weights.length) {
      throw new Error(
        `Length of inputs (${inputActivations.length}) does not match length of weights (${this.weights.length})`
      );
    }
