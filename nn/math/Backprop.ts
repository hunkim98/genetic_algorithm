import { NNNetworkObj } from "../Network";
...
export class Backprop {
  ...
  get costArray(): number[] {
    var costArr: number[] = [];
    var output =
      this.network.layers[this.network.layers.length - 1].nodeActivity;
    if (output.length !== this.expected.length) {
      throw new Error(
        `Ouput length (${output.length}) and expected length (${this.expected.length}) don't match!`
      );
    }
   ...
   * @memberof Backprop
   */
  deltaPrevActivityOnZ(
    layerIndex: number,
    currentNodeIndex: number,
    prevNodeIndex: number
  ) {
    return this.network.layers[layerIndex].nodes[currentNodeIndex].weights[
      prevNodeIndex
    ];
  }
  ...
  deltaActivityOnCost(
    ...
    return (
      this.network.layers[layerIndex].nodes[nodeIndex].activity -
    ...
    for (var layerIndex = layerCount; layerIndex >= 0; layerIndex--) {
      ...
      for (var nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
        ...
        if (layerIndex === layerCount) {
          var activityOnCost = this.deltaActivityOnCost(
            layerIndex,
            nodeIndex,
            expectedIndex
          );
        ...
        var weightCount =
          this.network.layers[layerIndex].nodes[nodeIndex].weights.length;
        for (var weightIndex = 0; weightIndex < weightCount; weightIndex++) {
          ...
          if (layerIndex > 0) {
            ...
            var prevActivityOnZ = this.deltaPrevActivityOnZ(
              layerIndex,
              nodeIndex,
              prevNodeIndex
            );
            var prevActivityOnCost =
              prevActivityOnZ * zOnActivity * activityOnCost;
