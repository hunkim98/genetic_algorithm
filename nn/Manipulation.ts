...
export class ManipulationMatrix {
  ...
  apply(
    ...
    for (var layerIndex = 0; layerIndex < layerCount; layerIndex++) {
      ...
      for (var nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++) {
        var weightCount =
          network.layers[layerIndex].nodes[nodeIndex].weights.length;
        for (var weightIndex = 0; weightIndex < weightCount; weightIndex++) {
          var weightDiff =
            this.changes[layerIndex][nodeIndex].weights[weightIndex] *
            weightLearningRate;
          network.layers[layerIndex].nodes[nodeIndex].weights[weightIndex] -=
            weightDiff;
        }
        var biasDiff =
          this.changes[layerIndex][nodeIndex].bias * biasLearningRate;
